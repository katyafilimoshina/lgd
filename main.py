...
st.markdown("- Задать алгебру Клиффорда,")
...
def if_preserves_grade1(element):
  '''
  Проверяет, сохраняет ли элемент подпространство ранга 1
  '''
...
def figure_reflections(vectors, start):
  ...
  for i in range(len(vectors_coordinates)):
    data_to_show.append(go.Scatter(x=vectors_coordinates[i][0], y=vectors_coordinates[i][1], marker = dict(size=8), line=dict(color=colors[i], width=2), name=f"Вектор {i+1}"))
    data_to_show.append(go.Scatter(x=hyper_coordinates[i][0], y=hyper_coordinates[i][1], line=dict(color=colors[i], width=2), marker = dict(symbol="asterisk"), name=f"Гиперплоскость {i+1}"))
...
if p + q != 0:
    ...
    try:
        ...
        st.write("Элемент введён некорректно. Проверь выбранную сигнатуру слева и введённый элемент.")
    except TypeError:
        st.write("Элемент введён некорректно. Посмотри примеры формата ввода выше.")
    except SyntaxError:
        st.write("Ошибка: введена пустая строка.")
else:
    st.markdown("Сначала выбери сигнатуру алгебры слева: мы не рассматриваем случай $$(p,q)=(0,0)$$.")
...
if p + q != 0:
    ...
    try: 
        ...
        st.write("Элемент введён некорректно. Проверь выбранную сигнатуру слева и введённый элемент. Посмотреть примеры формата ввода можно выше.")
    except TypeError:
        st.write("Элемент введён некорректно. Посмотри примеры формата ввода выше.")
